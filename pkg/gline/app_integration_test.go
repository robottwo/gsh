...
func TestApp_Analytics_Integration(t *testing.T) {
	...
	logger.Sugar().Debugf("Initial analytics entries: %d", len(analytics.entries))

	// Test analytics recording manually without calling Gline to avoid hang and extra entries
	model := initialModel(
		"> ",
		[]string{},
		"",
	...
	// Diagnostic logging: Before adding entry
	logger.Sugar().Debugf("Before NewEntry - analytics entries: %d", len(analytics.entries))

	// Simulate calling analytics
	err := analytics.NewEntry(model.lastPredictionInput, model.lastPrediction, model.result)
	require.NoError(t, err)

	// Diagnostic logging: After adding entry
	logger.Sugar().Debugf("After NewEntry - analytics entries: %d", len(analytics.entries))
	for i, entry := range analytics.entries {
		logger.Sugar().Debugf("Entry %d: input=%s, prediction=%s, result=%s", i, entry.predictionInput, entry.prediction, entry.result)
	}
