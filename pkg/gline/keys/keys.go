...
func getAllSupportedKeyPresses() map[string]KeyPress {
	...
	for _, ks := range sequenceMapping {
		for _, k := range ks {
...
func ParseKeybind(s string) (KeyPress, bool) {
	terms := strings.Split(strings.ToUpper(s), "+")
	ctrl := false
	alt := false
	shift := false
	rune := ""
	...
	var segments []string
	if ctrl {
		segments = append(segments, "CTRL")
	}
	if alt {
		segments = append(segments, "ALT")
	}
	if shift {
		segments = append(segments, "SHIFT")
	}
	segments = append(segments, rune)
	keyString := strings.Join(segments, "+")

	k, ok := allSupportedKeyPresses[keyString]
	return k, ok
}
